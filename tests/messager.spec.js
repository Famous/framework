...
test('Messager', function(t) {
    ...
    t.test('Messager sends messages to self (i.e., \'$root\' selector)', function(st){
        ...
        BEST.Application.deploy('famous:html-element', 'body', function(app) {
            var rootNode;
            var newContent = 'hello world';

            rootNode = app.graph.rootNode;
            st.equal(rootNode.stateManager.get('content'), '', 'Initial state of famous:html-element is empty string');
    ...
    t.test('Messager sends messages using CSS selector', function(st){
        ...
        BEST.Application.deploy('famous-demos:clickable-square-with-label', 'body', function(app) {

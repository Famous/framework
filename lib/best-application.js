...
BestApplication.prototype.processDOMNodes = function(DOMNodes, parentBestNode) {
    for (var i = 0; i < DOMNodes.length; i++) {
        ...
        if (bestNode.isComponent) {
            this.processDOMNodes(bestNode.childrenRoot.childNodes, bestNode);
            ...
            while (bestDOMNode.firstChild) {
                bestDOMNode.removeChild(bestDOMNode.firstChild);
            }
            while (bestNode.childrenRoot.childNodes[0]) {
                bestDOMNode.appendChild(bestNode.childrenRoot.childNodes[0]);
            }

            // Handle yield
            if (bestNode.surrogateRoot.childNodes.length) {
                if (bestNode.publicEvents[YIELD_KEY]) {
                    bestNode.eventManager.send(YIELD_KEY, bestNode.surrogateRoot.childNodes);
                }
                else {
                    YieldConduit.handleYield(bestNode, bestNode.surrogateRoot);
                }
            }

            bestNode.stateManager.triggerGlobalChange(STATE_AUTOTRIGGER_RE);
        }
    }
};

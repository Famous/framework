...
StateManager.prototype.onUpdate = function onUpdate (time) {
    ...
    if (this._physicsEngine) {
        ...
        for (key in this._physicsStore) {
            var position = this._physicsEngine.getTransform(this._physicsStore[key]).position;
...
StateManager.prototype._addToPhysicsStore = function _addToParticleStore (name, particle) {
...
StateManager.prototype._createPhysicsParticle = function _createPhysicsParticle (name, position) {
    ...
    this.set(name, this._physicsEngine.getTransform(particle).position);
...
StateManager.prototype.applyPhysicsForce = function applyPhysicsForce (forceName, bodyNames, options) {
    ...
    for (var i = 0, len = bodyNames.length; i < len; i++) {
        ...
        var bodyPosition = this.get(bodyName);

        if (isArray(bodyPosition)) {
            bodies.push(this._createPhysicsParticle(bodyName, bodyPosition));
        }
        else {
            console.error('The state \'' + bodyName + '\' should be an array corresponding to a position.');
        }
    }

    this._physicsEngine.add(new physics[capitalizeFirstLetter(forceName)](bodies, options));

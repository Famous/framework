        ...
        for (key in this._physicsStore) {
            if (this._physicsStore.hasOwnProperty(key)) {
                ...
                if (physicsInstance instanceof physics.Particle) {
                    var position = this._physicsEngine.getTransform(physicsInstance).position;
...
StateManager.prototype.applyPhysicsForce = function applyPhysicsForce (forceName, bodyNames, options) {
    ...
    var force = new physics[capitalizeFirstLetter(forceName)](bodies, options);

    switch (forceName) {
        case 'drag':
        case 'gravity1D':
        case 'rotationalDrag':
            this._physicsEngine.add(force);
    ...
    switch (constraintName) {
        ...
        case 'hinge':
            if (bodies.length !== 2) {
                ...
                this._physicsEngine.add(new constraint(bodies[0], bodies[1], options));
            }
            break;
        case 'collision':
        case 'curve':
            if (bodies.length < 2) {
                ...
                this._physicsEngine.add(new constraint(bodies, options));
            }
            break;
        default:

FamousFramework.component('famous-demos:lightbox', {
  behaviors: {
    ...
    '.cell': {
      ...
      'size': function(maxColumns, options, width, height){
        var columns = maxColumns || options.columns;
        var rows = options.rows;
        return [(width - ((columns - 1) * options.gutter)) / columns,
                (height - ((rows - 1) * options.gutter)) / rows];
      },
      'style': {
  ...
  events: {
    ...
    '$self': {
      'size-change': function($state, $famousNode){
        ...
        var maxColumns = Math.ceil(width / 320);
        $state.set('maxColumns', maxColumns);
        ...
        for(var i = 0; i < numberOfElements; i++){
          ...
          $state.set(['positions', i], originalPositions[i], {duration: 1 + transitionStagger * i, curve: 'linear'})
            .thenSet(['positions',i],[(gutterOffsetX + cellWidth * thisColumn) || 0, (gutterOffsetY + cellHeight * thisRow) || 0], transition);
        }
      }
    }
  },
  states: {
    ...
    transition: {curve: 'inOutElastic', duration: 650},

...
var DEPENDENCIES_NAME = 'dependencies';
...
var EMPTY_OBJECT = { type: OBJECT_EXPRESSION, properties: [] };
...
function getConfig(ast) {
    ...
    return configs[0] || EMPTY_OBJECT;
}

function getDependencies(config) {
    var deps = {};
    eachProperty(config, function(key, value, property) {
        if (key === DEPENDENCIES_NAME) {
            eachLiteral(value, function(depKey, depValue) {
                deps[depKey] = depValue;
            });
        }
    });
    return deps;
...
function getDefinition(ast) {
    var modInvoke = getModuleInvocation(ast);
    if (!modInvoke || !modInvoke.arguments) {
        return EMPTY_OBJECT;
    }
    else {
        return modInvoke.arguments[1] || EMPTY_OBJECT;
    }
}

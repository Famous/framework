...
function getPayload(behavior, component) {
...
Behaviors.selfMessagingBehavior = function selfMessagingBehavior(behavior, component) {
    var payload = getPayload(behavior, component);
...
famous:core:ui-element
    behaviors
        #view
            align
 */
Behaviors.targetMessagingBehavior = function messagingBehavior(behavior, component, targets) {
    var payload = getPayload(behavior, component);
    var uid;
    var targetComponent;
    for (var i = 0; i < targets.length; i++) {
        uid = getUID(targets[i]);
        targetComponent = DataStore.getComponent(uid);
        targetComponent.events.triggerPublicEvent(behavior.name, payload, uid);
    }
}

Behaviors.targetDirectBehavior = function directBehavior(behavior, component, targets) {
    // TODO
    console.log('direct: ', behavior.name, behavior);
}

'use strict';

var ArrayUtils = require('framework-utilities/array');

var COMPONENT_NAME_DELIMITER = ':';
var COMPONENT_FILE_SUFFIX = '.js';
var FILE_MATCHER_RE = /(\.js|\.html|\.jade)$/i;
var FILE_NAME_DELIMITER = '.';
var FWD_SLASH = '/';
var BLANK_STR = '';
var STR_TYPE = 'string';
var DEPS_TEST_REGEX = /([\w-_.]+:)+(([\w-_.]+(?=[\s|>|\/]))|([\w-_.]+(?=:)))/ig;

function Finder(options) {
    if (!options) {
        options = {};
    }
    this.componentsHost = options.componentsHost || 'http://localhost:8357';
    this.componentsPathPrefix = this.componentsHost + '/';
    this.widgetPrefix = options.widgetPrefix || 'best-local';
    this.componentPath = componentPath.bind(this);
    this.componentURL = componentURL.bind(this);
    this.findDependencies = findDependencies.bind(this);
    this.subcomponentURLs = subcomponentURLs.bind(this);
    this.subcomponentType = subcomponentType.bind(this);
    this.findTreeDependencies = findTreeDependencies.bind(this);

}

function componentPath(name) {
    var parts = name.split(COMPONENT_NAME_DELIMITER);
    if (parts[0] === this.widgetPrefix) {
        return window.location.origin;
    }
    else {
        return this.componentsPathPrefix.concat(parts.join(FWD_SLASH));
    }
}

function componentURL(name) {
    var path = this.componentPath(name);
    var parts = path.split(FWD_SLASH);
    var lastPart = parts[parts.length - 1];
    var fullParts = parts.concat(lastPart);
    var fullPath = fullParts.join(FWD_SLASH);
    return fullPath.concat(COMPONENT_FILE_SUFFIX);
}

function findTreeDependencies(tree) {
    var matches = (tree || BLANK_STR).match(DEPS_TEST_REGEX) || [];
    var fixedMatches = [];
    for (var i = 0; i < matches.length; i++) {
        if (matches[i].indexOf('localhost') === -1) {
            fixedMatches.push(matches[i]);
        }
    }
    return fixedMatches;
}

function findBehaviorDependencies(behaviorGroups) {
    var dependencies = [];
    if (!behaviorGroups || behaviorGroups.length === 0) {
        return dependencies;
    }
    for (var selector in behaviorGroups) {
        var behaviors = behaviorGroups[selector];
        for (var behaviorName in behaviors) {
            if (DEPS_TEST_REGEX.test(behaviorName)) {
                var parts = behaviorName.split(COMPONENT_NAME_DELIMITER);
                var head = parts.splice(0, parts.length - 1);
                dependencies.push(head.join(COMPONENT_NAME_DELIMITER));
            }
        }
    }
    return dependencies;
}

function findEventDependencies(eventGroups) {
    var dependencies = [];
    if (!eventGroups || eventGroups.length === 0) {
        return dependencies;
    }
    for (var selector in eventGroups) {
        var events = eventGroups[selector];
        for (var eventName in events) {
            if (DEPS_TEST_REGEX.test(eventName)) {
                var parts = eventName.split(COMPONENT_NAME_DELIMITER);
                var head = parts.splice(0, parts.length - 1);
                dependencies.push(head.join(COMPONENT_NAME_DELIMITER));
            }
        }
    }
    return dependencies;
}

function findDependencies(definition) {
    var dependencies = [];
    dependencies = dependencies.concat(findTreeDependencies(definition.tree));
    dependencies = dependencies.concat(findBehaviorDependencies(definition.behaviors));
    dependencies = dependencies.concat(findEventDependencies(definition.events));
    return ArrayUtils.union(dependencies, []);
}

function subcomponentURLs(name, definition) {
    var output = {};
    for (var key in definition) {
        var value = definition[key];
        if (typeof value === STR_TYPE) {
            if (value.match(FILE_MATCHER_RE)) {
                var basePath = this.componentPath(name);
                output[key] = [basePath, value].join(FWD_SLASH);
            }
        }
    }
    return output;
}

function subcomponentType(url) {
    var parts = url.split(FILE_NAME_DELIMITER);
    return parts[parts.length - 1];
}

function Finder(options) {
    if (!options) {
        options = {};
    }
    this.componentsHost = options.componentsHost || 'http://localhost:8357';
    this.componentsPathPrefix = this.componentsHost + '/';
    this.widgetPrefix = options.widgetPrefix || 'best-local';
    this.componentPath = componentPath.bind(this);
    this.componentURL = componentURL.bind(this);
    this.findDependencies = findDependencies.bind(this);
    this.subcomponentURLs = subcomponentURLs.bind(this);
    this.subcomponentType = subcomponentType.bind(this);
}

module.exports = Finder;

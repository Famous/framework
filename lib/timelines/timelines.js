...
Timelines.prototype.start = function start() {
    ...
    this._pausedTimelines[this._currentTimelineName] = false;
...
Timelines.prototype.halt = function halt() {
    var timelineStateName = '__' + this._currentTimelineName + 'Time';
    var currentTimelineTime = this._states.get(timelineStateName);

    // duration is needed until all states are stored in Transitionables
    this._states.set(timelineStateName, currentTimelineTime, {duration:0});

    this._pausedTimelines[this._currentTimelineName] = true;
}

Timelines.prototype.resume = function resume() {
    var timelineStateName = '__' + this._currentTimelineName + 'Time';
    var time = this._timelineGroups[this._currentTimelineName].duration;
    var timeLeft = time - this._states.get(timelineStateName);

    this._states.set(timelineStateName, time, {duration: timeLeft});

    this._pausedTimelines[this._currentTimelineName] = false;
}

Timelines.prototype.isPaused = function isPaused() {
    return this._pausedTimelines[this._currentTimelineName] || false;
}

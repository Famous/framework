...
function loadComponent(name, partial, final) {
    return loadSubcomponents(name, partial, function(full) {
        var fixed = Shorthand.process(full);
        Bundle.addDefinition(name, fixed);
        var dependencies = Finder.findDependencies(fixed);
...
function loadDependencies(names, final) {
    ...
    return loadSources(names, function(sources) {
        return Evaluator.evalSources(sources, loadComponent, HelperFunctions, final);
    });
}
...
function loadEntrypoint(name, final) {
    return loadSource(name, function(source) {
        return Evaluator.evalEntrypoint(source, loadComponent, HelperFunctions, final);
    });
}

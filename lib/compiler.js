...
function loadDependencies(definition, config, packages, finish) {
    dependencies.load(definition, config, packages, function(err, deps) {
        var keys = Object.keys(deps);
        async.map(keys, function(moduleName, cb) {
            compile(moduleName, deps[moduleName], packages, function(err) {
                cb(err);
            });
        }, function(err) {
            finish(err);
...
function compile(name, files, packages, after, finish) {

...
ControlFlow.prototype.processSelfContainedFlows = function processSelfContainedFlows(blueprint, uid) {
    ...
    this.processIfBehaviors(blueprint, expandedBlueprint, uid);
    this.processRepeatBehaviors(blueprint, expandedBlueprint, uid);
    this.emit('setExpandedBlueprint', expandedBlueprint);
}

ControlFlow.prototype.processIfBehaviors = function processIfBehaviors(blueprint, expandedBlueprint, uid) {
    var ifBehaviors = this.controlFlow[IF_KEY];
    for (var i = 0; i < ifBehaviors.length; i++) {
        this.processIfBehavior(ifBehaviors[i], blueprint, expandedBlueprint, uid);
    }
}

ControlFlow.prototype.processIfBehavior = function processIfBehavior(behavior, blueprint, expandedBlueprint, uid) {
    var component = DataStore.getComponent(uid);
    var payload = Behaviors.getPayload(behavior, component);
    if (!this.ifData[behavior.selector]) {
        this.ifData[behavior.selector] = undefined;
    }
    if (payload !== this.ifData[behavior.selector]) {
        // TODO
    }
}

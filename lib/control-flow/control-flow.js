...
ControlFlow.prototype.getRepeatedElements = function getRepeatedElements() {
    var result = [];
    var item;
    for (var selector in this.repeatData) {
        item = this.repeatData[selector];
        for (var parentIndex = 0;  parentIndex < item.parentUIDs.length; parentIndex++) {
            for (var nodeIndex = 0; nodeIndex < item.parentUIDs[parentIndex].repeatedNodes.length; nodeIndex++) {
                result.push(item.parentUIDs[parentIndex].repeatedNodes[nodeIndex]);
            }
        }
    }
    return result;

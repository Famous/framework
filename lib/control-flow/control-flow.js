...
// This function is used to slot in repeated nodes into the correct place
// creates a wrapper if the selector for the $yield isn't $self
// otherwise, it appends it directly
function doesInjectableMatchYieldValue(injectable, value) {
    if (typeof value === 'string') {
        if (value[0] === '.') {
            return injectable.classList.contains(value.slice(1));
        }
        else if (value[0] === '#') {
            return injectable.id === value.slice(1);
        }
        else {
            return injectable.tagName.toLowerCase() === value.toLowerCase();
        }
    }
    else {
...
// creates a wrapper if the selector for the $yield isn't $self
// otherwise, it appends it directly
function processDynamicYield(injectable, expandedBlueprint, controlFlowDataMngr) {
    var yieldBehaviors = controlFlowDataMngr.behaviors[YIELD_KEY];
    var behavior;
    var value;
    var parentNode;
    ...
    for (var i = 0; i < yieldBehaviors.length; i++) {
        ...
        if (behavior.selector === SELF_KEY && value !== false) {
            ...
            if (value || doesInjectableMatchYieldValue(injectable, value)) {
                ...
                VirtualDOM.addNode(VirtualDOM.clone(injectable), clonedPort);
                return clonedPort;
            }
        }
    }
...
// Add $index to element being repeated unless its already add

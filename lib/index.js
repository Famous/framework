...
StateManager.prototype.setState = function setState(key, value, transition) {
    ...
    //    if (key.length > 1) {
    //        key.shift();
    //        this.set(key, value, transition);
    //    }
    //    else if (isString(key[0])) {
    //        console.log(key[0]);
    //        this.set(key[0], value, transition)
...
StateManager.prototype.getState = function getState(key) {
    return isArray(key) ? traverse(this._state, key) : this._state[key];
}

function traverse(object, path) {
    if (object.hasOwnProperty(path[0])) {
        if (path.length === 1) return object[path[0]];
        else return traverse(object[path.shift()], path);
    }
    else {

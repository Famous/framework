...
function StateManager(initialState) {
    ...
    //TODO:  use Clock.update (to patch into rAF)
    //       instead of uber-ridiculously-desperation-hacky setInteval.
    //       Problem appears to be that the included
    //       Clock (via require) doesn't match up with the
    //       famous-standalone one-off bundle (different versions?, different references to Clock?)
    //       As a result, Clock.update (which allows a function to be invoked upon rAF) isn't firing.
    //       To fix, should find a way to get a reference to the main Clock instance (from Famous included in the core framework) here.
...
StateManager.prototype.setState = function setState(key, value, transition) {
    ...
    if(transition){
        ...
        if(!(this._transitionables[key] instanceof Transitionable)){
            this._transitionables[key] = new Transitionable(previousState);
        }else{
            //we already have a t9able; halt it so we can start anew
            this._transitionables[key].halt();
        }
        this._transitionables[key].set(value, transition);
    }else{
        //if this used to be a t9able and isn't anymore,
        //clean up by halting and removing
        if(this._transitionables[key] && (this._transitionables[key] instanceof Transitionable)){
            this._transitionables[key].halt();
        }

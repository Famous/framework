...
function compile(name, tag, files, finish) {
    var packages = {};
    var afterFn = null;
    compiler.compile(name, tag, files, packages, afterFn, finish);
}
...
function getURL(name, tag) {
    var base = path.base(name);
    var folder = path.join(base, FOLDER, tag, BUNDLE_NAME);
    return path.join(store.baseURL, folder);
}

...
function BehaviorConduit(bestApplication, bestNode) {
    ...
    this.repeatData = {};
    /*
    this.repeatData keeps track of bestNodes created through
    the '$repeat' behavior in order to diff behavior payloads
    and only create/delete nodes from a parent if the payload
    is different.
    ...
    this.repeatData = {
        '#example': {
            payload : [{a: 1}, {a: 2}, {a: 3}],
            payloadEquality : [false, false, false],
            nodeStore : {
                parentUID_A : [bestNode1, bestNode2, bestNode3],
                parentUID_B : [bestNode4, bestNode5, bestNode6]
            },
            initialized : true
        }
    }
...
BehaviorConduit.prototype.buildRepeatController = function(behavior, behaviorDeps) {
    return function() {
        // Find/create repeatDataBucket (stores payload, payloadEquality, repeated nodes)
        if (!this.repeatData[behavior.selector]) {
            this.repeatData[behavior.selector] = {};
            this.repeatData[behavior.selector].nodeStore = {};
            this.repeatData[behavior.selector].initialized = false; // Flag
        }
        var dataBucket = this.repeatData[behavior.selector];

        // Calculate payload and payloadEquality
        ...
        dataBucket.payloadEquality = ArrayUtils.checkElementEquality(behaviorPayload, dataBucket.payload || []);
        dataBucket.payload = behaviorPayload;
        ...
        // Find parent 'ports' (i.e., parent DOMNode of repeated items)
        var repeatedElements = fetchBehaviorTargets(this.bestNode.DOMNode, behavior.selector);
        for (i = 0; i < repeatedElements.length; i++) {
            parentUID = repeatedElements[i].parentNode.getAttribute(UID_KEY);
            if (!dataBucket.nodeStore[parentUID]) {
                dataBucket.nodeStore[parentUID] = [];
            }

            // Add elements that initially appear in tree that haven't been
            // processed through the repeatController
            if (!dataBucket.initialized) {
                bestNode = this.bestApplication.bestNodes[repeatedElements[i].getAttribute(UID_KEY)];
                dataBucket.nodeStore[parentUID].push(bestNode);
            }
        }

        // Delete repeated nodes if the payload has changed
        // debugger;
        var repeatedNodes;
        var parentBestNode;
        var parentDOMNodeFromTree;
        var DOMNodeBluePrint;
        var nodeCount;
        for (parentUID in dataBucket.nodeStore) {
            parentBestNode = this.bestApplication.bestNodes[parentUID];
            repeatedNodes = dataBucket.nodeStore[parentUID];
            nodeCount = Math.max(dataBucket.payloadEquality.length, dataBucket.nodeStore[parentUID].length);

            for (i = 0; i < nodeCount; i++) {
                if (!dataBucket.payloadEquality[i] || !dataBucket.initialized) {
                    // Delete element
                    if (repeatedNodes[i]) {
                        repeatedNodes[i].remove();
                        repeatedNodes[i] = null;
                    }
                    // TODO -> Recursively remove refences from application's bestNode store
                    // this.bestApplication.bestNodes[uid] = null;

                    // Create element
                    if (dataBucket.payload[i]) {
                        parentDOMNodeFromTree = this.bestNode.treeSignature.querySelector('[uid=\'' + parentUID + '\']');
                        DOMNodeBluePrint = parentDOMNodeFromTree.querySelector(behavior.selector).cloneNode(DO_CLONE_ATTRIBUTES);
                        bestNode = this.bestApplication.processDOMNodes([DOMNodeBluePrint], parentBestNode, true)[0];
                        repeatedNodes[i] = bestNode;

                        bestNode.eventManager.sendMessages(dataBucket.payload[i]);
                        parentBestNode.DOMNode.appendChild(bestNode.DOMNode);
                        this.bestApplication.setupSceneGraph(bestNode.DOMNode, parentBestNode.famousNode);
                    }
                }
            }
        }

        this.repeatData[behavior.selector].initialized = true;

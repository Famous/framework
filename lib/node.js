...
function Node(domNode) {
    ...
    if (this.isComponent) {
        ...
        this.eventChannel.lifecycleMessage(PRELOAD_KEY, this);
        this.behaviorConduit = new BehaviorConduit(this);
        this.domStore = new DomStore(this);
        this.eventChannel.lifecycleMessage(POSTLOAD_KEY, this);
    }
}

Node.prototype.prepare = function(famousParentNode) {
    this.famousNode = famousParentNode.addChild();
    this.clearChildren();
...
Node.prototype.processRepeatInfo = function() {
    var domNode = this.domStore.domNode;
    var infoStr = domNode.getAttribute(REPEAT_INFO_KEY);
    var info;
    var index;
    var repeatPayload;
    if (infoStr) {
        info = JSON.parse(infoStr);
        index = info[INDEX_KEY];
        repeatPayload = info[REPEAT_PAYLOAD_KEY];
        domNode.removeAttribute(REPEAT_INFO_KEY);
    }
    else {
        index = 0;
        repeatPayload = 0;
    }
    this.stateManager.set(INDEX_KEY, index);
    this.stateManager.set(REPEAT_PAYLOAD_KEY, repeatPayload);
};
...
Node.prototype.teardown = function() {
    this.eventChannel.lifecycleMessage(PREUNLOAD_KEY, this);
    ...
    this.eventChannel.lifecycleMessage(POSTUNLOAD_KEY, this);
};

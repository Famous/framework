 ...
 */
EventHandler.prototype.addListener = function(type, callback, context, id) {
 ...
 */
EventHandler.prototype.removeListener = function(type, callbackOrId) {
    ...
    while (length--) {
        if (listeners[length][identifier] === callbackOrId) listeners.splice(length, 1);
    }
    return this;
}
 ...
 * @return {EventHandler} this
 */
EventHandler.prototype.trigger = function(type, event) {
...
EventHandler.prototype.subscribe = function(source) {
    if (~this._subscribedIds.indexOf(source)) return;
    var self = this;

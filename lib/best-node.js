...
function BestNode(name, bestDOMNode, parentBestNode, definition, FamousApplication) {
    ...
    if (definition) {
        ...
        this.treeSignature = DOM_PARSER.parseFromString(treeString, TREE_DATA_TYPE).body;
        ...
        this.eventManager = new EventManager(this.stateManager, this.publicEvents, this.name);
...
BestNode.prototype.setState = function setState(state) {
    for(var key in state) {
        this.stateManager.set(key, state[key]);
    }
}

BestNode.prototype.remove = function remove() {
    // remove state manager, event manager, event conduit, behavior conduit

    // Remove from Famo.us scene graph
    if (this.parentNode) {
        this.parentNode.famousNode.removeChild(this.famousNode);
        this.parentNode.DOMNode.removeChild(this.DOMNode);
    }
};

BestNode.prototype.toString = function toString() {

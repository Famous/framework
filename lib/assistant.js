...
Assistant.prototype.watchDirectory = function(baseDir, subDir) {
    ...
    Chokidar.watch(watchDir, { ignored: this.options.chokidarIgnored, ignoreInitial: true }).on('all', function(event, filename) {
        ...
        if (moduleFullDir) {
            ...
            this.syncSingle(baseDir, moduleRelativeDir, function(err, result) {
                ...
                else console.log('synced ' + moduleRelativeDir);
            }.bind(this));
        }
    }.bind(this));
};

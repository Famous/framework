...
Events.prototype.setupProxyEvent = function setupProxyEvent(event, targetRoot, uid) {
    ...
    var proxy = event.name.slice(lastDelimIdx + 1);
    ...
    for (var i = targets.length - 1; i >= 0; i--) {
        ...
        bestComponent = DataStore.getComponent(uid);
        triggerArgs = Injector.getArgs(triggerParams, payload, uid);

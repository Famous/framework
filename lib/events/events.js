...
Events.prototype.setupProxyEvent = function setupProxyEvent(event, targets, uid) {
    ...
    for (var i = 0; i < targets.length; i++) {
        ...
        var specialIndicies = getIndexAndRepeatPayloadParamIndex(event.params);
        if (specialIndicies) {
            ...
            payload.listener = function(component, eventPayload) {
                ...
                listenerArgs[specialIndicies.$indexIndex] = component.states.get(INDEX_KEY);
                listenerArgs[specialIndicies.$repeatPayloadIndex] = component.states.get(REPEAT_PAYLOAD_KEY);
...
function getDescendantEvents(eventName, descendantEvents) {
    var events = [];
    var i;
    var event;
...
Events.prototype.setupDispatchedEvent = function setupDispatchedEvent(event, targets, uid) {
    ...
    for (var i = 0; i < targets.length; i++) {
        ...
        component.events.dispatcher.on(eventName, function(component, message) {
            var eventsToFire = getDescendantEvents(eventName, this.eventStore[DESCENDANT_KEY][event.selector] || {});
            var args;
            for (i = 0; i < eventsToFire.length; i++) {
                event = eventsToFire[i];
                args = Injector.getArgs(event.params, message, uid);
                if (specialIndicies) {

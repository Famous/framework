...
Component.prototype.processDynamicRepeat = function processDynamicRepeat(behavior) {
    ...
    var repeatedElements = this.controlFlow.getRepeatedElements();
    var newComponentCreated = false;
    var result;
    for (var i = 0; i < repeatedElements.length; i++) {
        result = this.processControlFlowMessage(repeatedElements[i]);
...
Component.prototype.processControlFlowMessage = function processControlFlowMessage(node) {
...
Component.prototype.processControlFlow = function processControlFlow(uid) {
    ...
    // elements are manually added
    var repeatedElements = this.controlFlow.getRepeatedElements();
    for (var i = 0; i < repeatedElements.length; i++) {
        repeatedElements[i].removeAttribute(CONTROL_FLOW_ACTION_KEY);
    }
}
...
Component.prototype.remove = function remove() {
    ...
    var expandedBlueprint = this.tree.getExpandedBlueprint();
    if (expandedBlueprint.parentNode) {
        expandedBlueprint.parentNode.removeChild(expandedBlueprint);
    }

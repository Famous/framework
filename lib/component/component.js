...
Component.prototype.processControlFlow = function processControlFlow(uid) {
    ...
    // Strip control-flow messages from node/expandedBlueprint since initially the elements are processed
    // through the non-dynamic channel and are instantiated via the the `updateChildren` method
    // set off by the `setChildrenRoot` event. This two tier flow is set up in order to ensure
    // that children are instantiated before parents so that the parent's behavior/event conduit

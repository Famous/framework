...
Component.prototype._maybeExposeStates = function() {
    if (this.config.expose) {
        for (var i = 0; i < this.config.expose.length; i++) {
            var field = this.config.expose[i];
            var value = this.states.get(field.key);
            if (value) {
                field.value = value;
            }
            else {
        ...
        this.events.dispatcher.on('$states-configured', function(payload) {
            if (payload && payload.detail) {
                if (this.uid === payload.detail.uid) {
                    this.states.set(payload.detail.key, payload.detail.value);
                }
            }
        }.bind(this));
        ...
        setTimeout(function() {
            this.events.dispatcher.emit('$states-exposed', {
                uid: this.uid,
                group: this.name,
                fields: this.config.expose
            });
        }.bind(this), 0);
    }
};

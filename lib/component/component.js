...
Component._processControlFlowMessage = function _processControlFlowMessage(node, progenitorExpandedBlueprint) {
    ...
    if (info) {
        ...
        if (info.message === CREATE_KEY) {
            var parentNode = DataStore.getComponent(info.parentUID);
            var parentExpandedBlueprint = parentNode.tree.getExpandedBlueprint();
            var parentControlFlowDataMngr = parentNode.controlFlowDataMngr;
            ...
            var nodeToAdd = ControlFlow.processDynamicYield(node, parentExpandedBlueprint, parentControlFlowDataMngr);
            var parentUID = JSON.parse(VirtualDOM.getAttribute(nodeToAdd, CONTROL_FLOW_ACTION_KEY)).parentUID;
            var baseNode = VirtualDOM.clone(nodeToAdd);
            VirtualDOM.removeChildNodes(baseNode);
            VirtualDOM.removeAttribute(baseNode, CONTROL_FLOW_ACTION_KEY);
            return new Component(baseNode, nodeToAdd, DataStore.getComponent(parentUID));
        }
            ...
            // Todo --> Add logic to remove the wrapper nodes

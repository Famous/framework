...
Deployer.prototype.deploy = function(name, tag, selector, cb) {
    var timeAwaited = 0;
    var readyInterval = setInterval(function() {
        timeAwaited += this.options.awaitInterval;

        if (this.componentsReady[name] && this.componentsReady[name][tag]) {
            clearInterval(readyInterval);
            var component = this.execute(name, tag, selector);
            if (cb) {
                cb(component);
            }
        }
        else {
            if (timeAwaited >= this.options.awaitTimeout) {
                clearInterval(readyInterval);
                console.error('Gave up waiting on `' + name + '` (' + tag + ')');
            }
        }
    }.bind(this), this.options.awaitInterval);
};

...
function create(behavior, node) {
    return function() {
        ...
        var targets = Targets.fetch(node.treeSignature, behavior.selector);
        for (var i = 0; i < targets.length; i++) {
            var target = targets[i];
            var targetNode = NodeStore.findNode(VirtualDOM.getUID(target));
            var targetEventManager = targetNode.eventManager;
            var payload = behavior.action.apply(null, args);
            targetEventManager.sendMessage(behavior.name, payload);
        }
    };
}

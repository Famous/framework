...
function initializeRepeat(info) {
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        for (var j = 0; j < info.payload.length; j++) {
            ...
            attachMessage(clone, info.payload[j]);
            parent.appendChild(clone);
        }
    }
}

function attachMessage(node, message) {
    var messages = JSON.stringify(message);
    node.setAttribute(MESSAGES_ATTR_KEY, messages);
}
        ...
        if (!payloadEquality[i]) {
            for (parentUID in parentData) {
                if (parentData[parentUID].nodes[i]) {
                    ...
                    if (info.payload[i] === undefined) debugger;
                    clone = VirtualDOM.clone(parentData[parentUID].blueprint);
                    attachMessage(clone, info.payload[i]);
                    clone.setAttribute(TREE_SIG_ATTR_KEY, VirtualDOM.getUID(info.domStore.treeSignature));
                    result.add.push({
                        parentUID: parentUID,

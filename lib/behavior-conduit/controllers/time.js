...
function create(behavior, node) {
    var states = node.stateManager.getStateObject();
    FRAMES[node.uid] = 0;
    return function(time) {
        ...
        for (i = 0; i < params.length; i++) {
            ...
            if (name === TIME_KEY) {
                ...
                args[i] = Args.fetchOne(name, states);
            }
        }
        var payload = behavior.action.apply(null, args);
        var targets = VirtualDOM.fetchTargets(node.domStore.domNode, behavior.selector);

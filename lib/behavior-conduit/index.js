...
BehaviorConduit.prototype.stateBehavior = function(behavior) {
    ...
    if (!behavior.params || behavior.params.length < 1) {
        this.node.stateManager.subscribeToGlobalChange(controller);
    }
...
BehaviorConduit.prototype.invertedBehavior = function(behavior) {
    var controller =  InvertedController.create(behavior, this.node);

    for (var i = 0; i < behavior.params.length; i++) {
        if (!(behavior.params[i] in REPEAT_BEHAVIOR_KEYS)) {
            this.node.stateManager.subscribeTo(behavior.params[i], controller);
        }
    }

    this.invertedBehaviors.push(controller);
    return controller;
}

BehaviorConduit.prototype.triggerInvertedBehaviors = function(behavior) {
    for (var i = 0; i < this.invertedBehaviors.length; i++) {
        this.invertedBehaviors[i]();
    }
}

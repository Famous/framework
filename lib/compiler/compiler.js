    ...
    Async.waterfall([
        ...
        function(cb) {
            findEntrypointFn(moduleName, compiledFiles, function(err, entrypointFound) {
                Errors.handle(err, 'failed-finding-entrypoint', { entrypointFound: entrypointFound }, cb, finalCB, function() {
...
Compiler.prototype.findDependencyKeys = function(objectAST, dependenciesList) {
    Es.eachObjectProperty(objectAST, function(keyName, _1, _2, valueObject, propObj) {
        if (this.helper.looksLikeDependency(keyName)) {
            dependenciesList.push(this.helper.getDependencyModuleName(keyName));
        }
...
function buildEventFunctionAST(key, value) {
    ...
    switch (functionKey) {
        case 'setter':
            var stateName = allEventFunctionFilters(key, filters);
            var fnString = '(function($state,$payload){$state.set(\'' + stateName + '\',$payload);})';
            var body = Es.parse(fnString).body[0];

var Babel = require('babel');
var Coffee = require('coffee-script');
var Jade = require('jade');
var Less = require('less');
var Path = require('path');
var Stylus = require('stylus');

var COMPILERS = {};

// String -> String
COMPILERS['.coffee'] = function(source, cb) {
    var result = Coffee.compile(source, {});
    cb(null, result);
};

// String -> String
COMPILERS['.jade'] = function(source, cb) {
    var result = Jade.compile(source)();
    cb(null, result);
};

// String -> String
COMPILERS['.js'] = function(source, cb) {
    var wrapped = '(function(){\n' + source + '\n}());';
    var result = Babel.transform(wrapped, {}).code;
    cb(null, result);
};

// String -> String
COMPILERS['.less'] = function(source, cb) {
    Less.render(source, function(err, output) {
        if (!err) {
            cb(null, output.css);
        }
        else {
            cb(err);
        }
    });
};

// String -> String
COMPILERS['.stylus'] = function(source, cb) {
    Stylus.render(source, {}, function(err, css) {
        if (!err) {
            cb(null, css);
        }
        else {
            cb(err);
        }
    });
};

// String -> String
COMPILERS['.html'] = function(source, cb) {
    cb(null, source);
};

// String -> String
COMPILERS['.json'] = function(source, cb) {
    cb(null, source);
};

// String -> String
COMPILERS['.css'] = function(source, cb) {
    cb(null, source);
};

// Process source for extname in `keys` with compiler for `value`
var ALIASES = {
    '.es': '.js',
    '.es6': '.js',
    '.es7': '.js',
    '.styl': '.stylus'
};

// The extname each type would normally get after compilation
var COMPILATION_EXTNAME_MAPPING = {
    '.styl': '.css',
    '.stylus': '.css',
    '.es': '.js',
    '.es6': '.js',
    '.es7': '.js',
    '.coffee': '.js'
};

for (var aliasExtname in ALIASES) {
    var compilerExtname = ALIASES[aliasExtname];
    COMPILERS[aliasExtname] = COMPILERS[compilerExtname];
}

// String -> Boolean
function hasCompilerFor(extname) {
    return !!COMPILERS[extname];
}

// String, String -> String
// Given a source code string and extname, compile the source
function compileSource(source, extname, cb) {
    COMPILERS[extname](source, function(err, result) {
        cb(null, result);    
    });
}

// String -> String
// Return the path that this file would have if it were compiled
function compiledPath(path) {
    var extname = Path.extname(path);
    var basename = Path.basename(path, extname);
    var remappedExtname = COMPILATION_EXTNAME_MAPPING[extname];
    if (remappedExtname) {
        return basename + remappedExtname;
    }
    else {
        return path;
    }
}

module.exports = {
    hasCompilerFor: hasCompilerFor,
    compiledPath: compiledPath,
    compileSource: compileSource
};
